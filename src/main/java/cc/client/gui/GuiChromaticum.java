...
import cc.utils.Notifier;
import cpw.mods.fml.client.FMLClientHandler;
import cpw.mods.fml.common.FMLCommonHandler;
import cc.api.ApiCore;
import cc.api.CategoryEntry;
import cc.api.DiscoveryEntry;
import cc.api.PageEntry;

import cc.api.StructureBlock;
import cc.api.StructureRecipe;
 ...
 */
public class GuiChromaticum extends GuiScreen {
    ...
    public static final ResourceLocation gui = new ResourceLocation(CCCore.modid,"textures/gui/research_book_generic.png");
    ...
    public void updateScreen()
    {
        ...
        if(!numberString.isEmpty() && pressDelay <= 0)
        {
            ...
            {
                Notifier.notifyCustomMod("ChromaCraft", "[ERROR]"+numberString+" is not a valid number!");
            }
            if(buttonID > -1)
            {
                buttonID -= 1;
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_)
    {
        ...
        {
            firstOpened = true;
            this.fontRendererObj = new FontRenderer(mc.gameSettings, new ResourceLocation("cc","textures/special/research_font.png"), mc.renderEngine, false);
        ...
        for (int ik = 0; ik < this.buttonList.size(); ++ik)
        {
            ...
            if(id == 0)
            {
                if(hover)
                {
                    ...
                    catStr.add(StatCollector.translateToLocal("cc.text.button.back"));
    ...
    public void drawPage_0(int mouseX, int mouseY)
    {
        ...
        if(currentPage == 0)
        {
            ...
            if(page.pageTitle == null || page.pageTitle.isEmpty())
            {
                ...
                {
                    added = "\u00a7l"+StatCollector.translateToLocal("cc.book.discovery_"+currentDiscovery.id+".name");
                }
                else
    ...
    public int drawStructureRecipe(int mouseX, int mouseZ, int k, int l, StructureRecipe toDraw)
    {
        try
        {
            this.fontRendererObj.drawString(StatCollector.translateToLocal("cc.txt.structure"), k+8, l+12, 0x222222);
            l += 5;
    ...
    {
        this.fontRendererObj.drawString(StatCollector.translateToLocal("cc.txt.shapedRecipe"), k+8, l+12, 0x222222);
    ...
    {
        this.fontRendererObj.drawString(StatCollector.translateToLocal("cc.txt.shapelessRecipe"), k+8, l+12, 0x222222);
        l += 5;
    ...
    public void drawDiscoveries(int mouseX, int mouseZ)
    {
        int k = (this.width - 256) / 2;
        int l = (this.height - 168) / 2;
        this.fontRendererObj.drawStringWithShadow("\u00a76 \u00a7l" + StatCollector.translateToLocal("cc.txt.book.startup"), k+6, l+10, 0xffffff);
        ...
        if(cat.name != null && !cat.name.isEmpty())
            ...
            this.fontRendererObj.drawStringWithShadow("\u00a76 " + StatCollector.translateToLocal("cc.book.category_"+currentCategory.id+".name"), k+6+128, l+10, 0xffffff);
        for (int ik = 0; ik < this.buttonList.size(); ++ik)
        {
            GL11.glColor3f(1, 1, 1);
            GuiButton btn  = (GuiButton) this.buttonList.get(ik);
            ...
            if(id > 2)
            {
                if(hover)
                {
                    ...
                    if(disc.name == null || disc.name.isEmpty())
                        discStr.add("\u00a7l"+StatCollector.translateToLocal("cc.book.discovery_"+disc.id+".name"));
                    else
                        discStr.add(disc.name);
                    if(disc.shortDescription == null || disc.shortDescription.isEmpty())
                        discStr.add("\u00a7o"+StatCollector.translateToLocal("cc.book.discovery_"+disc.id+".desc"));
                    else
                        discStr.add(disc.shortDescription);
                    if(disc.isNew)
                        discStr.add("\u00a76"+"New");
                    discStr.add(StatCollector.translateToLocal("cc.txt.contains")+disc.pages.size()+StatCollector.translateToLocal("cc.txt.pages"));
    ...
    public void drawCategories(int mouseX, int mouseZ)
    {
        ...
        int l = (this.height - 168) / 2;
        this.fontRendererObj.drawStringWithShadow("\u00a76 \u00a7l" + StatCollector.translateToLocal("cc.txt.book.startup"), k+6, l+10, 0xffffff);
        this.fontRendererObj.drawStringWithShadow("\u00a76 \u00a7l" + StatCollector.translateToLocal("cc.txt.book.categories"), k+134, l+10, 0xffffff);
        this.fontRendererObj.drawStringWithShadow("\u00a76" + StatCollector.translateToLocal("cc.txt.book.containedKnowledge"), k+16, l+25, 0xffffff);
        int tier = bookTag.getInteger("tier");
        for(int i = 0; i <= tier; ++i)
        {
            this.fontRendererObj.drawStringWithShadow("\u00a77-\u00a7o" + StatCollector.translateToLocal("cc.txt.book.tier_"+i), k+16, l+35+(i*10), 0xffffff);
        }
        this.fontRendererObj.drawStringWithShadow("\u00a76" + StatCollector.translateToLocal("cc.txt.book.edition"), k+16, l+90, 0xffffff);
        this.fontRendererObj.drawString("\u00a72" + FMLCommonHandler.instance().findContainerFor(CCCore.instance).getDisplayVersion()+"r", k+16, l+100, 0xffffff);
        k += 128;
        this.mc.renderEngine.bindTexture(gui);
        GL11.glColor3f(1, 1, 1);
        RenderHelper.disableStandardItemLighting();
        ...
        for (int ik = 0; ik < this.buttonList.size(); ++ik)
        {
            ...
            if(tier >= reqTier)
            {
                if(hover)
                {
                    List<String> catStr = new ArrayList<String>();
                    if(cat.name == null || cat.name.isEmpty())
                        catStr.add("\u00a7l"+StatCollector.translateToLocal("cc.book.category_"+cat.id+".name"));
                    else
                        catStr.add(cat.name);
                    if(cat.shortDescription == null || cat.shortDescription.isEmpty())
                        catStr.add("\u00a7o"+StatCollector.translateToLocal("cc.book.category_"+cat.id+".desc"));
                    else
                        catStr.add(cat.shortDescription);
                    catStr.add(StatCollector.translateToLocal("cc.txt.contains")+cat.discoveries.size()+StatCollector.translateToLocal("cc.txt.entries"));
                    this.func_146283_a(catStr, mouseX, mouseZ);
                }
            }
        }
        ...
        if(toDraw != null)
        {
            ...
            }else
            {
                ...
                if(hover)
                {
                    ...
                    {
                        catStr.add(EnumChatFormatting.ITALIC + StatCollector.translateToLocal("cc.txt.is.press"));
    ...
    public void drawSB(StructureBlock drawable, int pX, int pZ, int mX, int mZ, int phase)
    {
        ...
        }else
        {
            ...
            {
                List<String> catStr = toDraw.getTooltip(this.mc.thePlayer, false);
                catStr.add(StatCollector.translateToLocal("cc.txt.relativePosition"));
